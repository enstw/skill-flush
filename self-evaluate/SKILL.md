...
1. **Investigate to reduce uncertainty.** Choose the scale yourself and time-box it proportional to the work's blast radius: a one-file refactor may need only a quick file/diff read, while a multi-system migration may justify a longer probe. Pick the cheapest checks that move the estimate most. The investigation itself is meta-work, not a loop - it is the preparation that makes the count more accurate.
